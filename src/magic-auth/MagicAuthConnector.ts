...
export class MagicAuthConnector extends Connector {
  ...
  constructor(magicConstructorArgs: MagicAuthConstructorArgs) {
    super(magicConstructorArgs.actions, magicConstructorArgs.onError)
    this.magic = new Magic('YOUR_API_KEY', {
      extensions: [new OAuthExtension()],
    });
    this.options = magicConstructorArgs.options
  }
  ...
  public async activate(): Promise<void> {
    const cancelActivation = this.actions.startActivation()
    try {
      console.log(this.initializeMagicAuthInstance())
    } catch (err) {
      cancelActivation()
    }
  }

  private async initializeMagicAuthInstance(): Promise<InstanceWithExtensions<SDKBase, OAuthExtension[]>> {
        const { magicAuthApiKey, networkOptions } = this.options
